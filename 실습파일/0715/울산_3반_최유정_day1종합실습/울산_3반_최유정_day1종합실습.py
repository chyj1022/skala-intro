...
[실행 방법]
※ 아래 명령은 이 파일이 있는 폴더(울산_3반_최유정_day1종합실습/) 안에서 실행합니다.
    pip install httpx pydantic pandas pyarrow pytest ruff
    python 울산_3반_최유정_day1종합실습.py        # 수집 + 저장/읽기 (weather.parquet 생성)
    pytest 울산_3반_최유정_day1종합실습.py -v      # 테스트 실행 (파일명 지정 필수!)
